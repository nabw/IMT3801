...
\example{
    ...
    \end{equation*}
    Note that if $u\geq 0$, then $a$ is elliptic. Then, if $u\in X\cap L^\infty \cap \{u\geq 0\}$ we have that $dF(u)\in\text{Inv}(X,Y)$ and we can use Theorem~\ref{thm:local-invertibility}.
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{theorem}[Brouwer fixed point]\label{thm:brouwer-fixed-point}
    Set $K$ a non-empty, compact and convex subset of a finite-dimensional Banach space. Then, every continuous $f:K\to K$ has a fixed point.
\end{theorem}
\example{
