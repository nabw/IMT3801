...
for all $v$. This function is typically called a \emph{test function}. The surface form suggest the boundary conditions: 
    $$ \int_{\partial\Omega}\underbrace{\gamma_D v}_\text{Dirichlet boundary condition} \underbrace{\gamma_N \grad u}_\text{Neumann boundary condition} \,ds,$$
...
\begin{lemma}[Poincaré inequality] There exists a positive constant $C$ such that for a non-empty portion of the subdomain $\Gamma \subseteq \partial\Omega$ it holds that
...
\begin{proof}
The main result to be used is the Bramble-Hilbert Lemma, which establishes in particular that if $B$ is a sufficiently big ball in $\Omega$ such that $\Omega$ is starred with respect to it, then the average over $B$ given by $\bar u = 1/|B|\int_B u\,dx$ satisfies
...
where the first tirm was controlled using the Friedrich inequality. The second term is controlled by forcing another triangle inequality: 
