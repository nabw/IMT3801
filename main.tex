...
\begin{proof}
    ...
    Now, let assume \(T\) is injective and it's range is closed. Then, because 
    ...
    show that \(T^{-1}\) is continuous. Then
    \begin{displaymath}
...
\begin{proof}
    ...
    \neq 0\). Which implies the right part of the equivalence.

    Now, lets assume the right part of the equivalence. By contradiction, if \(T\)
...
\begin{theorem}(Gen. Lax-Milgram)
    Consider \(H_1\), \(H_2\) hilbert spaces and a bounded bilinear form 
    \(B:H_1\to H_2\). Then, there exists a unique \(u\in H_1\) for each 
    \(F\in H_2'\) such that \[B(u,v) = F(v),\quad \forall v \in H_2\] if and only if 
    \begin{enumerate}
        \item \(\exists \alpha > 0\) such that \[\sup_{v\in H_2}
        \frac{B(u,v)}{\lVert v \rVert_{H_2}} \ge \alpha \lVert u \rVert_{H_1}
        \quad \forall u\in H_1\quad \text{ (surjective) }\]
        \item \[\sup_{u\in H_1} B(u,v) > 0 
        \quad \forall v\in H_2,\,\, v\neq 0
        \quad \text{ (injective) }\]
    \end{enumerate}
\end{theorem}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
