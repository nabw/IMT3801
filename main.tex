...
A saddle point problem has the form:
\begin{displaymath}
    \begin{bmatrix}
        A & B^{T}\\ 
        B & 0
...
To grasp the importance of this formulations, let's first see
some examples:
\begin{itemize}
    ...
    \end{cases}
    \end{displaymath}
    From where it's clear that \(A=-\Delta\), \(B=\dive\).
    ...
    \end{cases}
    \end{displaymath}
    Acting the equation over test function \(\tau\) and 
    \(v\) we end up with the saddle point problem. 
    \begin{displaymath}
    \begin{cases}
        (\sigma, \tau) - (u, \dive \tau) &= 0
        \hspace{3em} \forall \tau \\ 
        (\dive \sigma, v) &= \langle f,v \rangle
        \hspace{1.2em} \forall v
    \end{cases}
    \end{displaymath}
    ...
    \begin{displaymath}
    \begin{cases}
        -\Delta u &= f \text{ over } \Omega \\ 
        u &= g \text{ over } \partial\Omega
    \end{cases}
    \end{displaymath}
...
as the \textbf{Babushka Breggy Theory}.
...
\begin{theorem}
    ...
    \begin{itemize}
        ...
        \item \(B\) satisfy the inf-sup condition.
    \end{itemize}
    Then, for each \((f,f)\) in \(H'\times Q'\) there exists
    a unique pair \((u,p)\in H\times Q\) such that~\eqref{mixed}
    holds. Moreover, there is a positive constant
    \(C=C(\|A\|, \| (\Pi A)^{-1}\|, B)\) such that
    \begin{equation}
        \| (u,p) \|
        \le
        C \left( \|f\| + \|g\| \right) 
...
\paragraph{Common example: Darcy} We will present a the worked example of Darcy's problem, to be analyzed using the LBB theory. Its weak form is given as follows: Consider $\vec H=H(\dive; \Omega)\cap \{\vec u\cdot \vec n=0\}$ and $Q=L^2(\Omega)$, then find $(u,p)$ in $H\times Q$ such that
    ...
    \end{aligned} \]
where $\ten K$ is symmetric and positive definite ($ k_1|\vec x|^2 \leq \vec x\cdot \ten K^{-1}\vec x \leq k_2 |\vec x|^2$), $f$ is in $\vec H'$ and $g$ is in $Q'$. We omit details regarding continuity as it is a simply application of the Cauchy-Schwartz inequality. The bilinear forms to be studied here are $a(\vec u, \vec v) = (\ten K^{-1}\vec u, \vec v)$ and $b(\vec v, q) = (\dive v, q)$.
    \begin{itemize}
        ...
        \[ a(\vec u, \vec v) = (\ten K^{-1}\vec u, \vec v). \]
