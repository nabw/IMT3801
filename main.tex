...
\example{
    Let $\psi(u)=\frac{1}{p}\int_\Omega u^p dx$, defined in $L^p$.
...
In calculus of variations $d^2 F(u)[h] = d^2 F(u)[h,h]$ is used for checking properties of a problem.
...
Then, we have the following local inversion
...
\example{
    ...
    \end{array}\right.
    \]
    where $p>1$, $h\in H^{-1}(\Omega)$, so $F:X\to Y$ with $X\coloneq H_0^1(\Omega)$ and $Y\coloneq H^{-1}(\Omega)$. We compute
