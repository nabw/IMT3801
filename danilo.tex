...
\textbf{Proposition:}
 ...
 is such that $a$ satisfies Gårding then $A$ is Fredholm. 
...
\begin{proof}
    ...
    The last two implies that $T\circ i$ is compact, as the composition of continuous and compact operator is compact. 
...
% \begin{myexample}{ }
%     \begin{itemize}
%         \item Helmholtz: \(A = -\Delta + k^2I\) is a Fredholm operator.
%         % , with,
%         % \begin{align*}
%         %     A:& H_0^1(\Omega) \to H^{-1}(\Omega)\\
%         %     K:& H_0^1(\Omega) \to H^{-1}(\Omega)\\
%         %     I:& H^1(\Omega)\to L^2(\Omega)\\
%         %     i:& L^2(\Omega) \to H^{-1}(\Omega) 
%         % \end{align*}
%         \item ADR: \(-\Delta u + \beta\dot\nabla u = f\), with $\beta\in L^\infty(\Omega)$. Then,
%         \[a(u,v) := (\nabla u,\nabla v) + (\beta\cdot\nabla u, v)\]
%     \end{itemize}
% \end{myexample}
...
where $\mathbb{A} = R^-1\circ A$ and $\mathbb{F} = R^{-1}f$. The stability estimate, finally gives, 
